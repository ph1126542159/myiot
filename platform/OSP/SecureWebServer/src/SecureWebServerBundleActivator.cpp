...
namespace
{
	Poco::Net::Context::VerificationMode verificationModeFromString(const std::string& value)
	{
		if (Poco::icompare(value, "none") == 0)
			return Poco::Net::Context::VERIFY_NONE;
		else if (Poco::icompare(value, "strict") == 0)
			return Poco::Net::Context::VERIFY_STRICT;
		else if (Poco::icompare(value, "once") == 0)
			return Poco::Net::Context::VERIFY_ONCE;
		else
			return Poco::Net::Context::VERIFY_RELAXED;
	}

	int disabledProtocolsFromString(const std::string& value)
	{
		int disabledProtocols = 0;
		Poco::StringTokenizer protocols(value, ",;", Poco::StringTokenizer::TOK_TRIM | Poco::StringTokenizer::TOK_IGNORE_EMPTY);
		for (const auto& protocol: protocols)
		{
			if (Poco::icompare(protocol, "sslv2") == 0)
				disabledProtocols |= Poco::Net::Context::PROTO_SSLV2;
			else if (Poco::icompare(protocol, "sslv3") == 0)
				disabledProtocols |= Poco::Net::Context::PROTO_SSLV3;
			else if (Poco::icompare(protocol, "tlsv1") == 0)
				disabledProtocols |= Poco::Net::Context::PROTO_TLSV1;
			else if (Poco::icompare(protocol, "tlsv1.1") == 0)
				disabledProtocols |= Poco::Net::Context::PROTO_TLSV1_1;
			else if (Poco::icompare(protocol, "tlsv1.2") == 0)
				disabledProtocols |= Poco::Net::Context::PROTO_TLSV1_2;
			else if (Poco::icompare(protocol, "tlsv1.3") == 0)
				disabledProtocols |= Poco::Net::Context::PROTO_TLSV1_3;
		}
	...
	void start(BundleContext::Ptr pContext)
	{
		...
		if (pWebServerDispatcherRef)
		{
			...
			if (port != 0)
			{
				...
				Poco::Util::AbstractConfiguration& config = *pPrefs->configuration();
				Poco::Net::Context::Ptr pSecureContext = new Poco::Net::Context(
					Poco::Net::Context::SERVER_USE,
					config.getString("openSSL.server.privateKeyFile", ""),
					config.getString("openSSL.server.certificateFile", ""),
					config.getString("openSSL.server.caConfig", ""),
					verificationModeFromString(config.getString("openSSL.server.verificationMode", "relaxed")),
					config.getInt("openSSL.server.verificationDepth", 9),
					config.getBool("openSSL.server.loadDefaultCAFile", true),
					config.getString("openSSL.server.cipherList", "ALL:!ADH:!LOW:!EXP:!MD5:@STRENGTH")
				);
				const int disabledProtocols = disabledProtocolsFromString(config.getString("openSSL.server.disableProtocols", ""));
				if (disabledProtocols != 0)
				{
					pSecureContext->disableProtocols(disabledProtocols);
				}

				SecureServerSocket svs(addr, maxQueued, pSecureContext);
